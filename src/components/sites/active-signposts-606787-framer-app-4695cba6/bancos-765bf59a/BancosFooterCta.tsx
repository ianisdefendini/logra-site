...
export function BancosFooterCta() {
  return (
    <section style={{ background: "#111111", padding: "120px 20px 60px" }}>
      <style>{`
        @keyframes bancos-streak-drift {
        ...
        .bancos-footercta-streak { animation: bancos-streak-drift 6s ease-in-out infinite; }
        .bancos-footercta-btn { transition: background 0.2s ease; }
        .bancos-footercta-btn:hover { background: #f0f0f0 !important; }
      `}</style>
      <div
        style={{
          maxWidth: 1400,
      ...
      >
        {streaks.map((streak, i) => (
          <span
            key={`streak-${i}`}
            className="bancos-footercta-streak"
            style={{
              position: "absolute",
              left: streak.left,
              top: streak.top,
        ...
        >
          Hablemos de cómo impulsar el crecimiento de tus clientes mejora la
          rentabilidad de tu cartera PyME.
        </h2>
        ...
        <a
          ...
          className="bancos-footercta-btn"
          style={{
